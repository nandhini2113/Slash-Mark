...
BEGIN
    -- Your SQL statements for managing the library go here
    -- Example: CREATE TABLE, INSERT, UPDATE, DELETE, etc.
...
CREATE PROCEDURE bookCopiesAtAllSharpstown (IN bookTitle VARCHAR(70), IN branchName VARCHAR(70))
BEGIN
    SELECT 
...
CREATE PROCEDURE bookCopiesAtAllBranches (IN bookTitle VARCHAR(70))
BEGIN
    SELECT 
        copies.book_copies_BranchID AS `Branch ID`, 
        branch.library_branch_BranchName AS `Branch Name`,
...
-- To execute the procedure with a default value
